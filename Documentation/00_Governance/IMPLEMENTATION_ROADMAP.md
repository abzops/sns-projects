...
- **P1-02C (`PRODUCTION SECURITY VERIFIED / LOCAL E2E BLOCKED ON DOCKER`)**: Workflow RPC Security, Search Path Hardening & Real E2E Closure. Fixed mutable search path on public `SECURITY INVOKER` functions (`start_process_instance`, `get_process_instance_progress`), refactored Process-Instance-aware overloads (`complete_responsible_part`, `reject_process_task`) to public INVOKER wrappers delegating to private DEFINER engines, revoked anonymous execute across all workflow RPCs, and eliminated all new Security Advisor warnings. [P1-02C Spec](../06_Implementation_Packages/Package_01_Core_Foundation/P1-02C_Workflow_RPC_Security_and_Real_E2E_Closure.md).

> [!NOTE]
> **Package 1 Status**: Production security and migrations are **`VERIFIED`**. Local PostgreSQL lifecycle E2E execution is **`BLOCKED ON DOCKER DESKTOP OPERATOR LAUNCH`**. Package 2 is **`NEXT`** once local E2E run is confirmed by operator.
