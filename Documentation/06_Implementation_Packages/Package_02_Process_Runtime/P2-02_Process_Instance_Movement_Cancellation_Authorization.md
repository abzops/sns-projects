...
2. **Process Instance Cancellation**: Idempotent, permanent cancellation of running Process Instances where completed tasks remain completed, unfinished tasks become cancelled, and no data is lost. The final evidence and internal-DAG immutability gaps are closed by [P2-02A](./P2-02A_Post_Cancellation_Immutability_Closure.md).
...
5. **Post-Cancellation Runtime Protection**: P2-02 introduced task-state guards for public completion/rejection flows and cancellation state partitioning. P2-02A adds the missing parent-instance guard to evidence submission and the independent running/cancelled-state guard to internal DAG advancement.
...
- **Post-Cancellation Guard**: P2-02 rejects completion, rejection, approval, and consultation through cancelled task state. The complete fail-closed guarantee for evidence and internal advancement is provided by [P2-02A](./P2-02A_Post_Cancellation_Immutability_Closure.md).
