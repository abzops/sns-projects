...
**Automated Verification**: `scripts/test-p7-01-financial-hierarchy.mjs` (43/43 assertions passed)
...
3. **Operational Visibility + Financial Authority Gate**: The RPC evaluates the intersection of operational visibility (Operational V1 / OV1-A) and financial authorization. If the caller cannot operationally view the requested Project, the RPC strictly returns `NULL` (fail-closed).
...
- **Grants**: Granted to `authenticated`; revoked from `PUBLIC` and `anon`. Zero new public `SECURITY DEFINER` functions added (maintaining the frozen Security Advisor baseline of 7).
...
| Persona | Operational Visibility | Container Summaries (`project_summary`, `phase_summaries`, `task_list_summaries`) | Task Read Model (`direct_spend`, `visible_rollup_spend`) | `financial_visibility` |
| :--- | :--- | :--- | :--- | :--- |
| **Workspace Owner / Admin** | Operational projects/tasks | Full container summaries for visible hierarchy | Full task rollups across visible tasks | `full` |
...
| **Finance Operator** | Operational projects/tasks | Full container summaries for operationally accessible projects | Full task rollups for operationally accessible tasks | `full` (or `NULL` if project is hidden) |
...
- **Fail-Closed Default**: Returns `{ hierarchyData: null, loading: false, error: null }` when disabled or unauthorized.
...
The automated test suite `scripts/test-p7-01-financial-hierarchy.mjs` validates 43 assertions across 5 comprehensive test suites:
...
- **Suite 2: Multi-Persona Fixtures & Hierarchy Setup** (12 personas, 2 workspaces, projects, phases, task lists, tasks, subtasks, process instances).
...
- **Suite 4: Detailed Test Invariants (Assertions A–S)**:
  ...
  - Assertion J: Cycle protection (`NOT child.id = ANY(path)`) and recursion depth limit (< 50) verified.
...
- **Suite 5: Frontend Hook & Normalization Tests (Assertions T–Y)**:
  - Normalization contract and risk band preservation.
  - Assertion T: Stale in-flight response rejection via generation token.
  - Assertion U: Render-time scope invariant.
  - Assertion V: Disabled hook fail-closed behavior.
  - Assertions W, X, Y: Project, User, and AuthorizationScopeKey switch fail-closed verified.
