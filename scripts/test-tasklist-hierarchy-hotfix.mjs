...
async function runTests() {
  ...
  // Verify ASRS has exactly 4 task lists (stray Test cleaned up)
  const { rows: asrsTaskLists } = await pgClient.query(`
    ...
    WHERE tl.project_id = '${asrsId}'
    ORDER BY m.position, tl.position;
  `);
  assert(asrsTaskLists.length === 4, `Test 2: ASRS project has exactly 4 Task Lists after stray cleanup (got ${asrsTaskLists.length})`);
  ...
  assert(deLists.length === 2, `Test 4: Milestone "Design & Engineering" has exactly 2 Task Lists (${deLists.map(tl => tl.name).join(', ')})`);

  const pvLists = asrsTaskLists.filter(tl => tl.milestone_name === 'Prototype & Validation');
  assert(pvLists.length === 2, `Test 5: Milestone "Prototype & Validation" has exactly 2 Task Lists (${pvLists.map(tl => tl.name).join(', ')})`);
  ...
  const { rows: allProjects } = await pgClient.query(`
    SELECT p.id, p.name,
      ...
      (SELECT count(*) FROM public.task_lists tl WHERE tl.project_id = p.id) as task_list_count,
  ...
  for (const p of allProjects) {
    assert(
      ...
      `Test 7: Project "${p.name}" has 2 Milestones, 4 Task Lists, and 8 Tasks`
    );
  }

  console.log('\n=== GROUP 3: CLIENT QUERY LOGIC SIMULATION (useTaskLists) ===');

  // Simulate useTaskLists step 1 query: select(*) from task_lists where project_id = ...
  const { rows: clientQuerySimulation } = await pgClient.query(`
    SELECT * FROM public.task_lists
    WHERE project_id = '${asrsId}'
    ORDER BY position ASC, created_at ASC;
  `);
  assert(clientQuerySimulation.length === 4, `Test 8: useTaskLists step 1 query returns 4 task lists for ASRS`);

  // Simulate step 2 query for task stats
  const { rows: taskStatsSimulation } = await pgClient.query(`
    ...
    WHERE t.project_id = '${asrsId}' AND t.task_list_id IS NOT NULL;
  `);
  assert(taskStatsSimulation.length === 8, `Test 9: useTaskLists step 2 query returns 8 tasks with status system_code`);
  ...
  // Verify task list count increments to 5
  const { rows: countAfterInsert } = await pgClient.query(`SELECT count(*)::int as c FROM public.task_lists WHERE project_id = '${asrsId}';`);
  assert(countAfterInsert[0].c === 5, `Test 13: Task List count reactively reflects new task list (5 lists)`);

  // Delete temporary task list
  await pgClient.query(`DELETE FROM public.task_lists WHERE id = '${tempTl[0].id}';`);
  const { rows: countAfterDelete } = await pgClient.query(`SELECT count(*)::int as c FROM public.task_lists WHERE project_id = '${asrsId}';`);
  assert(countAfterDelete[0].c === 4, `Test 14: Task List count returns to exactly 4 after deletion`);
