...
async function runDP1D1Tests() {
  ...
  assert(tlCount === 12, `Test 53: Task Lists = 12 (got ${tlCount})`);
  assert(ctlCount === 12, `Test 54: Custom Task Lists = 12 (got ${ctlCount})`);
  assert(dtlCount === 0, `Test 55: Defined Task Lists = 0 (got ${dtlCount})`);
  assert(tCount === 24, `Test 56: Tasks = 24 (got ${tCount})`);
  assert(dtCount === 0, `Test 57: Defined Tasks = 0 (got ${dtCount})`);
  assert(subCount === 48, `Test 58: Subtasks = 48 (got ${subCount})`);
  assert(raciLiveCount === 72, `Test 59: RACI = 72 (got ${raciLiveCount})`);
